...
interface SummaryData { totalOrders: number; totalRevenue: number; avgOrderValue: number; }
interface OrderTypeRow { order_type: string; count: number; revenue: number; }
interface TopItem { name: string; quantity_sold: number; revenue: number; }
...
function getDateRange(preset: DateRangePreset, customStart?: string, customEnd?: string) {
  ...
  switch (preset) {
    case 'today': start = new Date(now.getFullYear(), now.getMonth(), now.getDate()); end = now; break;
    case 'week':
      const day = now.getDay(); const diff = day === 0 ? 6 : day - 1;
      start = new Date(now.getFullYear(), now.getMonth(), now.getDate() - diff); end = now; break;
...
export default function ReportsView({ slug, theme, currencySymbol }: Props) {
  ...
  const hasReportsView = hasPermission(perms, role, 'reports:view');
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      if (topItemsRes.ok && topItemsRes.data) {
        ...
        for (const item of topItemsRes.data) {
          ...
          prev.qty += item.quantity; prev.rev += item.quantity * Number(item.price_at_order);
    ...
    setLoading(false);
  }, [preset, customStart, customEnd, slug]);

  useEffect(() => {
    if (!isLoaded || !hasReportsView) return;
    fetchReports();
  }, [isLoaded, hasReportsView, fetchReports]);
  ...
  if (!hasReportsView) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-50">
        <div className="text-center"><h2 className="text-2xl font-bold text-gray-400 mb-2">Reports</h2><p className="text-gray-300">You do not have permission to view reports.</p></div>
      </div>
    );
  }
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(['today', 'week', 'month', 'custom'] as DateRangePreset[]).map((p) => (
              <button key={p} onClick={() => setPreset(p)}
                className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors ${preset === p ? 'text-white' : 'bg-white text-gray-600 border border-gray-300 hover:bg-gray-50'}`}
            ...
            <button onClick={fetchReports} disabled={loading} className="px-3 py-1.5 rounded text-xs font-semibold text-white disabled:opacity-50" style={{ backgroundColor: theme.primaryColor }}>{loading ? '...' : 'Refresh'}</button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {orderTypeBreakdown.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead><tr className="text-gray-500 border-b"><th className="text-left py-2 font-medium">Type</th><th className="text-right py-2 font-medium">Orders</th><th className="text-right py-2 font-medium">Revenue</th></tr></thead>
                    <tbody>
                      ...
                      <tr className="font-semibold text-sm"><td className="py-2 text-gray-700">Total</td><td className="py-2 text-right">{orderTypeBreakdown.reduce((s, r) => s + r.count, 0)}</td><td className="py-2 text-right">{currencySymbol}{orderTypeBreakdown.reduce((s, r) => s + r.revenue, 0).toFixed(2)}</td></tr>
                    </tbody>
                  </table>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {topItems.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead><tr className="text-gray-500 border-b"><th className="text-left py-2 font-medium">Item</th><th className="text-right py-2 font-medium">Sold</th><th className="text-right py-2 font-medium">Revenue</th></tr></thead>
                    <tbody>
                      {topItems.map((item, i) => (
                        <tr key={item.name} className="border-b border-gray-100">
                          <td className="py-1.5">
                            <div className="flex items-center gap-2">
                              <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold text-white ${i < 3 ? '' : 'bg-gray-300'}`} style={i < 3 ? { backgroundColor: theme.primaryColor } : {}}>{i + 1}</span>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              {dailyRevenue.length === 0 ? (
                ...
                <div className="relative">
                  <div className="flex mb-1"><div className="w-12 flex-shrink-0" /><div className="flex-1 flex justify-between text-[10px] text-gray-400"><span>{currencySymbol}{Math.round(maxDailyRevenue)}</span><span>{currencySymbol}{Math.round(maxDailyRevenue / 2)}</span><span>{currencySymbol}0</span></div></div>
                      ...
                      return (
                        <div key={day.date} className="flex-1 flex flex-col items-center min-w-0">
                          <div className="flex-1 w-full flex items-end justify-center">
                            <div className="w-full max-w-[40px] rounded-t transition-all" style={{ height: `${Math.max(pct, 2)}%`, backgroundColor: theme.primaryColor, opacity: 0.7 + (pct / 100) * 0.3 }} title={`${day.date}: ${currencySymbol}${day.revenue.toFixed(2)}`} />
                          </div>
                          <span className="text-[9px] text-gray-400 mt-1 truncate w-full text-center">{barCount > 14 ? dayLabel.split('/')[1] : dayLabel}</span>
                        </div>
                      );
                    })}
                  </div>
