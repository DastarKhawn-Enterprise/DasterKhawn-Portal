...
export default function ExpensesView({ supabaseUrl, supabaseAnonKey, theme }: Props) {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    setLoading(false);
  }, [authReady, getSupabaseClient, getDateRange, categoryFilter]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingExpense) {
        const { error: err } = await client.from('expenses').update(payload).eq('id', editingExpense.id);
        if (err) { setError(err.message); setSaving(false); return; }
        setExpenses((prev) => prev.map((e) => (e.id === editingExpense.id ? { ...e, ...payload } as Expense : e)));
      } else {
        const { data, error: err } = await client.from('expenses').insert(payload).select('*').single();
        if (err) { setError(err.message); setSaving(false); return; }
        if (data) setExpenses((prev) => [data as unknown as Expense, ...prev]);
      }
      setShowForm(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      const client = await getSupabaseClient();
      const { error: err } = await client.from('expenses').delete().eq('id', deleteId);
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-400 mb-2">Expenses</h2>
          <p className="text-gray-300">You do not have permission to view expenses.</p>
        </div>
      </div>
    );
  }
              ...
              {(['this-month', 'this-week', 'custom'] as DatePreset[]).map((p) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-semibold transition-colors ${
                    datePreset === p ? 'text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
          ...
          <div className="bg-white md:rounded-lg shadow-xl w-full md:max-w-md md:mx-4 p-6 rounded-t-xl md:max-h-[90vh] md:overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0" value={formAmount} onChange={(e) => setFormAmount(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded text-sm" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} placeholder="e.g. July electricity bill" className="w-full px-3 py-2 border border-gray-300 rounded text-sm" />
              </div>
