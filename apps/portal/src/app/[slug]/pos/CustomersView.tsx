...
export default function CustomersView({ supabaseUrl, supabaseAnonKey, theme }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (decoded) setCanEdit(hasPermission(decoded.permissions, decoded.tenant_role, 'orders:create'));
      } catch (e) {}
    })();
  }, [authReady, getToken]);
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    setLoading(false);
  }, [authReady, getSupabaseClient, search]);
  ...
  const fetchOrderHistory = useCallback(async (customerId: string) => {
    ...
    try {
      ...
      if (data) setOrderHistory(data as unknown as PastOrder[]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingCustomer) {
        const { error: err } = await client.from('customers').update(payload).eq('id', editingCustomer.id);
        ...
        const { data, error: err } = await client.from('customers').insert(payload).select('*').single();
        if (err) { setError(err.message); setSaving(false); return; }
        if (data) setCustomers((prev) => [...prev, data as unknown as Customer]);
      }
      setShowForm(false);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-4">
          <input
            ...
            className="w-full md:w-80 px-3 py-2 border border-gray-300 rounded text-sm"
          />
        </div>
          ...
          <div className="bg-white md:rounded-lg shadow-xl w-full md:max-w-md md:mx-4 p-6 rounded-t-xl md:max-h-[90vh] md:overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded text-sm" autoFocus />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Phone</label>
                <input type="text" value={formPhone} onChange={(e) => setFormPhone(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded text-sm" />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Email</label>
                <input type="email" value={formEmail} onChange={(e) => setFormEmail(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded text-sm" />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Notes</label>
                <textarea value={formNotes} onChange={(e) => setFormNotes(e.target.value)} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded text-sm" />
              </div>
